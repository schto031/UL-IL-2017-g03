...
public class ICrashCoordGUIController extends AbstractAuthGUIController {
	...
	*/
    ...
    @FXML
    private Button bttnSmsIdentification;
...
//	}
	...
	public void logon() {
		if(txtfldCoordLogonUserName.getText().length() > 0 && psswrdfldCoordLogonPassword.getText().length() > 0){
			try {
				userController.oeLogin(txtfldCoordLogonUserName.getText(), psswrdfldCoordLogonPassword.getText()).getValue();
...
public void smscontrol() {
		...
		if(txtfldCoordLogonUserName.getText().length() > 0 && txtfldCoordSmsCode.getText().length() > 0){
			
			try {
	/*1*/			if (userController.oeSmscontrol(txtfldCoordLogonUserName.getText(), txtfldCoordSmsCode.getText()).getValue()){
